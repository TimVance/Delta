...
echo '<div class="modal-overlay"><div class="js_cart_one_click cart_one_click modal">';
echo '<button class="modal__close js--modal-close" type="button">
        <span class="visually-hidden">Закрыть окно</span>
        <span class="modal__close-icon"></span>
      </button>
<form method="POST" action="" class="modal__content form js_cart_one_click_form cart_one_click_form ajax" enctype="multipart/form-data">
<p class="modal__title">'.$this->htmleditor('<insert value="Заказ техники"></insert>').'</p><br>
...
if (! empty($result["rows_param"]))
{
	foreach ($result["rows_param"] as $row)
	{
		...
		echo '<div class="form-item order_form_param'.$row["id"].'">';
		...
		switch ($row["type"])
		{
			...
			case 'text':
				echo '<div class="form-label infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input class="form-field" type="text" name="p'.$row["id"].'" value="'.$value.'">';
				break;

			case "phone":
				echo '<div class="form-label infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input class="form-field" type="tel" name="p'.$row["id"].'" value="'.$value.'">';
				break;

			case "email":
				echo '<div class="form-label infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<input class="form-field" type="email" name="p'.$row["id"].'" value="'.$value.'">';
				break;

			case 'textarea':
				echo '<div class="form-label infofield">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
				<textarea class="form-field" name="p'.$row["id"].'" rows="3">'.$value.'</textarea>';
				break;
...
echo '
<input class="form-submit button" type="button" value="'.$this->diafan->_('Заказать', false).'">';
echo '<div class="errors error"'.($result["error"] ? '>'.$result["error"] : ' style="display:none">').'</div>';

echo '<br /><br /><div class="privacy_field">'.$this->diafan->_('Отправляя форму, я даю согласие на <a href="%s">обработку персональных данных</a>.', true, BASE_PATH_HREF.'privacy'.ROUTE_END).'</div>';
