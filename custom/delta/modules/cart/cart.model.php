...
class Cart_model extends Model
{
	...
	private function prepare_form_data()
	{
		...
		if($this->cache["delivery"] && ! empty($this->result["discounts"]["delivery"]))
		{
			foreach($this->cache["delivery"] as &$dl)
			{
				if(empty($this->result["discounts"]["delivery"][$dl["id"]]))
				{
					continue;
				}
				$dl["discount_total"] = $this->result["discounts"]["delivery"][$dl["id"]];
				$dl["discount_total"]["currency"] = $this->diafan->configmodules("currency", "shop");
			}
			unset($dl);
		}
