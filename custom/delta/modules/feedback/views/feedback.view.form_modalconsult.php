...
echo '
<div class="modal-overlay consult">
<div id="free-consultation" class="feedback_form modal">
...
<p class="modal__title">'.$this->diafan->_("Бесплатная консультация", false).'</p><br>
...
if(! empty($result["rows"]))
{
	foreach ($result["rows"] as $row) //вывод полей из конструктора форм
	{
		...
		switch ($row["type"])
		{
			...
			case 'text':
				echo '<div class="form-label">'.$row["name"].($row["required"] ? '' : '').':</div>
			...
			case 'textarea':
				echo '<div class="form-label">'.$row["name"].($row["required"] ? '<span style="color:red;">*</span>' : '').':</div>
...
//Кнопка Отправить
echo '<input type="submit" value="'.$this->diafan->_('Отправить', false).'" class="form-submit button">';
