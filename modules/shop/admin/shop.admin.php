...
class Shop_admin extends Frame_admin
{
	...
	public function edit_variable_discounts()
	{
		...
		// скидки
		$discounts = DB::query_fetch_all("SELECT * FROM {shop_discount} WHERE trash='0' AND threshold_cumulative=0 AND threshold=0 AND threshold_goods=0 AND threshold_count=0 AND delivery_id=0 AND payment_id=0 AND (date_finish=0 OR date_finish>%d)", mktime(date("H"), 0, 0));
	...
	public function save_variable_discounts()
	{
		...
		foreach ($discounts as $i => &$d)
		{
			if($d["objects"]["brands"] && ! array_intersect($d["objects"]["brands"], $brands)
			|| $d["objects"]["cats"] && ! array_intersect($d["objects"]["cats"], $cats)
			// || $d["objects"]["goods"] && ! in_array($this->diafan->id, $d["objects"]["goods"])
			)
			{
				unset($discounts[$i]);
				continue;
		...
		foreach ($discounts as $i => $d)
		{
			...
			else
			{
				if (count($d["objects"]["goods"]) > 1)
				{
					DB::query("DELETE FROM {shop_discount_object} WHERE discount_id=%d AND good_id<>0 AND good_id<>%d", $d["id"], $this->diafan->id);
				}
				if(! DB::query_result("SELECT id FROM {shop_discount_object} WHERE discount_id=%d AND good_id=%d", $d["id"], $this->diafan->id)) {
					DB::query("INSERT INTO {shop_discount_object} (discount_id, good_id) VALUES (%d, %d)", $d["id"], $this->diafan->id);
				}
				DB::query("UPDATE {shop_discount} SET act='1' WHERE id=%d", $d["id"]);
			}
			$this->diafan->_shop->price_calc(0, $d);
		}
	}
