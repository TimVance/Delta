				...
				if(! empty($additional_costs[$row["good_id"]]))
				{
					foreach($additional_costs[$row["good_id"]] as $a)
					{
						...
						<div class="item__in">
							...
							<div class="num no_important ipad summ_additional_cost">'.$this->format_summ($a["summ"] * $count).'
							</div>
...
<div class="num"><input name="discount_summ" value="'
			...
			// TO_DO: при оформлении заказа - Cart_action::order() - общая скидка
			// была равномерно распределена и уже присутствует в стоимости товаров - функция $this->diafan->_cart->get().
			// В связи с этим запрещено редактирование размера общей скидки,
			// иначе общая скидка будет применена повторно при сохранении результатов редактирования заказа в административной части сайта.
			//
			// .'" size="4" type="text"></div>
			// 		<div class="num no_important ipad discount_summ">'.($this->diafan->values("discount_summ") ? '-'.$this->format_summ($this->diafan->values("discount_summ")) : '').'</div>
			// 		<div class="num no_important ipad"></div>
			//
			// 	</div>
			// </li>';
			//
			.'" size="4" type="text" disabled="disabled" class="disable"></div>
					<div class="num no_important ipad discount_summ">'.($this->diafan->values("discount_summ") ? '-'.$this->format_summ($this->diafan->values("discount_summ")) : '').'</div>
					<div class="num no_important ipad"></div>
	...
	public function save_variable_goods()
	{
		...
		{
			$new_prices_all = DB::query_fetch_key("SELECT price_id, price, old_price, good_id, discount_id, id FROM {shop_price} WHERE id IN (%s)", $this->diafan->filter($_POST["new_prices"], "integer"), "id");
		}
		$good_ids = array();
		if($rows)
		{
